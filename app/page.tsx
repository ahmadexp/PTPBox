...
type ResearchPayload = {
  ...
  auto_tune: {
    ...
    recommendation?: { kp: number; ki: number; rms_ns: number; tail_rms_ns?: number; peak_ns: number; score: number; safe: boolean; overshoot_ratio?: number; settling_s?: number; model_spectral_radius?: number | null; model_settling_s?: number | null; robust_penalty?: number | null };
    baseline?: { kp: number; ki: number; rms_ns: number; tail_rms_ns?: number; peak_ns?: number; score: number; model_spectral_radius?: number | null };
    frontier?: Array<{ kp: number; ki: number; score: number; rms_ns: number; model_spectral_radius?: number | null; robust_penalty?: number | null }>;
    guardrails?: { peak_limit_ns?: number; max_model_spectral_radius?: number; uses_identified_arx?: boolean; uses_active_hinfinity?: boolean; live_apply?: string };
  };
...
function buildResearchModel(history: HistoryPoint[], nodes: ClockNode[]): ResearchPayload {
  ...
  return {
    ...
    auto_tune: {
      ...
      recommendation: { kp: .6, ki: .2, rms_ns: Math.max(1, (nodes[nodes.length - 1]?.rms ?? 20) * .776), tail_rms_ns: 14.4, peak_ns: Math.abs(currentOffset) * 1.2, overshoot_ratio: 1.18, settling_s: 22.5, model_spectral_radius: .82, model_settling_s: 17.6, robust_penalty: 1.4, score: 24.8, safe: true },
      baseline: { kp: .7, ki: .3, rms_ns: nodes[nodes.length - 1]?.rms ?? 20, tail_rms_ns: 18.9, peak_ns: Math.abs(currentOffset) * 1.4, model_spectral_radius: .91, score: 31.9 },
      guardrails: { peak_limit_ns: 20_000, max_model_spectral_radius: .995, uses_identified_arx: true, uses_active_hinfinity: true, live_apply: "stage-only" },
      frontier: [{ kp: .6, ki: .2, score: 24.8, rms_ns: 18.1, model_spectral_radius: .82, robust_penalty: 1.4 }, { kp: .8, ki: .2, score: 25.4, rms_ns: 18.6, model_spectral_radius: .86, robust_penalty: 1.8 }, { kp: .6, ki: .35, score: 27.1, rms_ns: 19.2, model_spectral_radius: .89, robust_penalty: 2.2 }],
    },
  ...
  return (
    <div className="intelligence-layout">
      ...
      <section className="instrument-panel tuner-panel">
        ...
        {research.auto_tune.recommendation ? <><div className="tune-recommendation"><div><span>RECOMMENDED PI</span><strong>Kp {research.auto_tune.recommendation.kp.toFixed(3)} · Ki {research.auto_tune.recommendation.ki.toFixed(3)}</strong><small>{research.auto_tune.safe_candidates}/{research.auto_tune.evaluated_candidates} replay-safe candidates · {research.auto_tune.guardrails?.live_apply ?? "stage-only"}</small></div><em><strong>{research.auto_tune.predicted_improvement_pct?.toFixed(1)}%</strong><span>predicted score improvement</span></em></div><div className="tune-evidence-grid"><div><span>Replay RMS</span><strong>{formatNanoseconds(research.auto_tune.recommendation.rms_ns)}</strong><small>baseline {research.auto_tune.baseline?.rms_ns == null ? "—" : formatNanoseconds(research.auto_tune.baseline.rms_ns)}</small></div><div><span>Tail RMS</span><strong>{research.auto_tune.recommendation.tail_rms_ns == null ? "—" : formatNanoseconds(research.auto_tune.recommendation.tail_rms_ns)}</strong><small>settling {research.auto_tune.recommendation.settling_s?.toFixed(1) ?? "—"} s</small></div><div><span>Peak / limit</span><strong>{formatNanoseconds(research.auto_tune.recommendation.peak_ns)}</strong><small>limit {research.auto_tune.guardrails?.peak_limit_ns == null ? "—" : formatNanoseconds(research.auto_tune.guardrails.peak_limit_ns)}</small></div><div><span>Model radius</span><strong>{research.auto_tune.recommendation.model_spectral_radius == null ? "—" : research.auto_tune.recommendation.model_spectral_radius.toFixed(3)}</strong><small>max {research.auto_tune.guardrails?.max_model_spectral_radius?.toFixed(3) ?? "—"}</small></div></div><div className="tune-guardrails"><span className={research.auto_tune.guardrails?.uses_identified_arx ? "on" : ""}>ARX plant</span><span className={research.auto_tune.guardrails?.uses_active_hinfinity ? "on" : ""}>H∞ penalty</span><span className="on">No live exploration</span><span>Robust penalty {research.auto_tune.recommendation.robust_penalty == null ? "—" : research.auto_tune.recommendation.robust_penalty.toFixed(2)}</span></div><div className="frontier-row">{research.auto_tune.frontier?.slice(0, 6).map((candidate, index) => <div key={`${candidate.kp}-${candidate.ki}`} style={{ height: `${35 + (1 - index / 6) * 45}%` }}><span>{candidate.score.toFixed(1)}</span><small>Kp {candidate.kp.toFixed(2)} · Ki {candidate.ki.toFixed(2)}</small></div>)}</div><button type="button" className="full-secondary" onClick={() => stageTune(research.auto_tune.recommendation!.kp, research.auto_tune.recommendation!.ki)}><SlidersHorizontal size={14} /> Stage recommendation for review</button></> : <div className="capability-empty"><Gauge size={20} /><div><strong>More samples required</strong><span>Optimization never explores gains on live hardware. It ranks candidates against captured data and enforces peak/error constraints.</span></div></div>}
      </section>
